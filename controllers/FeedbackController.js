...
router.get('/', getUser, auth(['atm', 'datm', 'ta', 'wm']), async (req, res) => {
	// All feedback
	try {
...
router.put('/approve/:id', getUser, auth(['atm', 'datm', 'ta', 'wm']), async (req, res) => {
	// Approve feedback
	try {
		const approved = await Feedback.findOneAndUpdate(
			{ _id: req.params.id },
			{
				approved: true,
			},
...
router.put('/reject/:id', getUser, auth(['atm', 'datm', 'ta', 'wm']), async (req, res) => {
	// Reject feedback
	try {
