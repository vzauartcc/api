		...
		questions.forEach((question, index) => {
			...
			if (question.isTrueFalse) {
				...
				const falseOption = question.options.find(
					(option) => option.text.toLowerCase() === 'false',
				);
...
router.post('/exams/:examId/start', getUser, async (req, res) => {
	...
	const latestAttempt = await ExamAttempt.findOne({ exam: examId, user: userId }).sort({
		createdAt: -1,
	...
	try {
		...
		const scoredResponses = responses.map((response) => {
			...
			const isCorrect = question.options.some(
				(option) => option._id.equals(response.selectedOption) && option.isCorrect,
			);
