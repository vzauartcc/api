...
router.get('/ins', getUser, auth(['atm', 'datm', 'ta', 'ins', 'mtr', 'ia']), async (req, res) => {
	try {
		...
		const allHomeControllers = await User.find({ member: true, rating: { $lt: 12 } }).select(
...
function getHighestCertificationBeforeQuarter(
	...
	const validCerts = certificationDates.filter((cert) => {
		...
		if (userCid === testUserCID) {
			console.log(
				`Checking cert: ${JSON.stringify(
					cert,
				)} | Valid: ${isValid} | In hierarchy: ${certHierarchy.includes(cert.code)}`,
			);
		}
...
async function fetchCertificationActivityTimes(users, startOfQuarter, endOfQuarter) {
	...
	const certTimeMap = Object.fromEntries(
		certActivities.map(({ _id, certTime }) => [_id, certTime]),
	);
...
router.get('/activity', getUser, auth(['atm', 'datm', 'ta', 'wm']), async (req, res) => {
	try {
		...
		const testUserCID = req.query.cid ? parseInt(requ.query.cid, 10) : 10000002; // 🔹 Replace with the specific user's CID
		...
		for (const user of users) {
			...
			if (highestCert) {
				...
				validPositions = Object.keys(atcPositionToCertMap).filter((position) =>
					higherCerts.includes(atcPositionToCertMap[position]),
				);
			}
		...
		const certTimes = await fetchCertificationActivityTimes(
			userCertMap,
			startOfQuarter,
			endOfQuarter,
		);
		...
		for (const user of users) {
			...
			if (user.cid === testUserCID) {
				console.log(
					`Test User ${user.cid} - Total Time: ${userData[user.cid].totalTime}s, Cert Time: ${
						userData[user.cid].certSpecificTime
					}s`,
				);
			}
		}
